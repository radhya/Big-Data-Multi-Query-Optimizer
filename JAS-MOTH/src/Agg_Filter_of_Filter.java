...
public class Agg_Filter_of_Filter {
  ...
  String c4;
   ...
   // Lineitem= Lineitem.aggregate(Aggregations.MAX,0).first(1);
       ...
       Lineitem1 = Lineitem1.filter(
            new FilterFunction<Lineitem>() {
                    ...
                    public boolean filter(Lineitem l) {
                        ...
                        return  Double.parseDouble( l.getField(0).toString())>=0.02 && Double.parseDouble( l.getField(0).toString())<=0.03  ;
       ...
       //.groupBy(0);//aggregate(Aggregations.SUM, 1).groupBy(0);
        ...
        Lineitem1.writeAsCsv("/home/hadoop/Desktop/Flink/Dataset/PlainResults/t6.csv", "\n", ",",WriteMode.OVERWRITE).setParallelism(1);
       env.execute();
...
public static  Tuple  get_Partsupp()
       ...
       DataSet<Lineitem> Lineitem=(DataSet<Lineitem>) env.readCsvFile("/home/hadoop/Desktop/Flink/Dataset/PlainResults/t3.csv")
