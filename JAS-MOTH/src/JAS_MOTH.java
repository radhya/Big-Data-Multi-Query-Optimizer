...
public class JAS_MOTH{
 ...
 public static String driverName = "org.apache.hive.jdbc.HiveDriver";
   ...
   * expermints types
   */
 //public static String table_name="My_Histogram_100t_old";
 //public static String table_name="My_Histogram_100t";
 public static String table_name="My_Histogram_10m";//"My_Histogram_100T";
  ...
  public static boolean JAS_MOTH=true; //JAS_MOTH true ,not JAS_MOTH false // this for run sequential all queries
  public static int MT_HistT_CostT=2; //1- Materlization 2- Histo 3-Cost
  public static int run_all=3; //1- run nonconcurrent 2-run concurrent 3-run all
  public static double rows_num_in_million=250;
  public static int test_run=1; //0-false test only 1- true test & run
  public static int second_run=0;  // 0-false 1-true
  public static int not_rerun_query_no=1;
  //for case study 1 not_rerun_query_no=3 ; for case study 2 run only concurrent
  // for 2nd case apply only concurrent and take  sequetail time for non concurrenent in cost model
  ...
  public static void main(String[] args ) throws SQLException, FileNotFoundException, IOException, InterruptedException {
      ...
      //Find_Nonderiviedquery(); Find_Sharing(0,query_num); Print_Sharing(); Find_Parent_Sharing(0);
       ...
       //System.out.println(get_stat_Histogram(1));
      // System.out.println(get_range_stat_Histogram(3,5));
       //int [] distinctvalues= new int[2];
       //int distinctvalues[]={1,8,5};
       //System.out.println(get_multi_stat_Histogram(distinctvalues));
  ...
  public static void Initi_Estimate_Results(int start, int end ) throws SQLException, IOException
  {
   ...
   public static void  Print_Sharing()
   {
    ...
    public static void Excute() throws SQLException, IOException, ClassNotFoundException, InterruptedException
    {
         List<ConcurrentListThread>  ThreadArr= new ArrayList<ConcurrentListThread>();
   
           /*
           * Now call sequentaily
           */
          // Excutor.drop_tables(SharingGraph_query_num);
       //ExcuteAsHive();
        // Excutor.drop_tables(SharingGraph_query_num);
         if(run_all==1) // run only nonconcurrent
        {
         ExcuteNonConcurrent();
        }
          if(run_all==2) // run only  concurrent
        {
         ExcuteConncurrent_JAS_MOTH();
        }
         if(run_all==3)  // run all nonconcurrent and concurrent
        {
         ExcuteNonConcurrent();
         ExcuteConncurrent_JAS_MOTH();
        }
           //ExcuteConncurrentTrees();
           
             /*
           * Now call thread to run each tree  sequentally concurrenly
           */
     ...
     public static void ExcuteAsHive() throws SQLException, IOException, ClassNotFoundException, InterruptedException
   {
       
           /*
           * Now call sequentaily
           */
              System.out.println("=============Run  as Hive All Sequentail list ======================="); 
          List <String> templist= new ArrayList <String> (); 
            for(int i=0; i<query_num;i++)
            {   String s="create table t"+String.valueOf(i+1)+" as "+SharingGraph.modifyquery(query_tokens_arr[i].query);
                templist.add(s);
            }
           System.out.println("===============================================================");
           Excutor.drop_tables(SharingGraph_query_num);
           
           run_time=Excutor.runSequtailQueryList(templist);
           
           System.out.println("Hive Sequtail Run***"+run_time);
   }//ExcuteaslHive
     
     
      public static void ExcuteNonConcurrent() throws SQLException, IOException, ClassNotFoundException, InterruptedException
   {
       nonconcurrentlist=SharingGraph.getNonConcurrentQueryList(query_tokens_arr,SharingGraph_arr, SharingGraph_query_num);//,estimated_cost_nonconcurrentlist);
       System.out.println("=============Non concurrentlist=======================");
       SharingGraph.printFinalOutputQueryList(nonconcurrentlist);
       // this to avoid run source table (root query) to save time 
       if(second_run==1)
               {
                   // skip from these queries
                 for(int i=0; i<not_rerun_query_no;i++)  
                   nonconcurrentlist.remove(0); // after removing the next 1 be at index zero
                   // nonconcurrentlist.remove(1);
            System.out.println("============= Updated Non concurrentlist=======================");
            SharingGraph.printFinalOutputQueryList(nonconcurrentlist);
       
       }
         if (test_run==1) // this to test and run
      {
          
       System.out.println("=============Run NonConcurrent list======================="); 
       // Excutor.drop_tables(SharingGraph_query_num);
       run_time=Excutor.runSequtailQueryList(nonconcurrentlist);
        System.out.println("nonconcurrentlist Sequtail Run= "+run_time);
      }   
   }//ExcuteNonConcurrent
     
     
   public static void ExcuteConncurrentTrees() throws SQLException, IOException, ClassNotFoundException, InterruptedException
   {
       
                /*
           * Now call thread to run each tree concurrenly
           */
           System.out.println("=============Concurrent Trees======================="); 
           Excutor.drop_tables(SharingGraph_query_num);
            List<List<String>> Allquerylist = new ArrayList<List<String>>();
            Allquerylist=SharingGraph.getAllConcurrentQueryList(SharingGraph_arr, SharingGraph_query_num);
         
            Excutor.runConcurrentList(Allquerylist);

           
   } // end of ExcuteConncurrentTrees 
   
   
   public static void ExcuteConncurrent_JAS_MOTH() throws SQLException, IOException, ClassNotFoundException, InterruptedException
   {
      if (JAS_MOTH) // only for JAS_MOTH system run concurrent 
      {
                /*
           * Now call thread to ExcuteConncurrentlist
           */
      concurrentlist=SharingGraph.getConcurrentQueryList(SharingGraph_arr, SharingGraph_query_num);
      System.out.println("=============concurrentlist in JAS_MOTH=======================");
      SharingGraph.printFinalOutputQueryList(concurrentlist);    
      
     
      if (test_run==1) // this to test and run
      {
           System.out.println("=============ExcuteConncurrentlist======================="); 
           Excutor.runConcurrent(concurrentlist);
      }
            
      }
      else // not JAS_MOTH system run sequentail
      {
      concurrentlist=SharingGraph.getConcurrentQueryList(SharingGraph_arr, SharingGraph_query_num);
          System.out.println("=============nonconcurrentlist in not JAS_MOTH=======================");
      SharingGraph.printFinalOutputQueryList(concurrentlist);    
      
     
      if (test_run==1) // this to test and run
      {
           System.out.println("=============ExcuteConncurrentlist not JAS_MOTH system======================="); 
           run_time=Excutor.runSequtailQueryList(concurrentlist);
           System.out.println("nonconcurrentlist in not JAS_MOTH system Sequtail Run= "+run_time);      }
  ...
  public static void sendtothreadOrderedByLevel()
    {
        
      // get each root query
         List <String> querylist= new ArrayList <String> ();
        for(int i=0; i<SharingGraph_query_num;i++)
        {
            if (!SharingGraph_arr[i].isshare )
            {
             querylist.add(SharingGraph_arr[i].Fullquery);
             System.out.println("non share query "+querylist.get(0));
            } // enf if share
            
            else if (SharingGraph_arr[i].isroot )
            {
              querylist=SharingGraph_arr[i].getsubquerylist(SharingGraph_arr);
             
            
        System.out.println("querylist of query "+(SharingGraph_arr[i].queryno+1) );
         for(int j=0; j<querylist.size();j++)
         {
           System.out.println(querylist.get(j));  
         }// end for j
         } // end if else
            querylist.clear();
        } //end for i
        
        // get quries orderd by level
        querylist=SharingGraph.getsubquerylistOrederedByLevel(SharingGraph_arr, query_num);
       System.out.println("querylist of query orderd by level");
         for(int j=0; j<querylist.size();j++)
         {
           System.out.println(querylist.get(j));  
         }// end for j
 ...
 // parse_run_multi_query function
    ...
    while (read.hasNextLine())
            {
                ...
                //init();
                parse_one_query(sql);
               // System.out.println(query_tokens_arr[query_num].columns_list);
                //query_tokens_arr[query_num].print_query_tokens();
                //System.out.println("get_columns_num ::"+query_tokens_arr[query_num].get_column_num("partkey"));
                  //set_multi_query_info();
 ...
 public static void parse_one_query(String sql) throws SQLException
    {
        try {
            ...
            Logger.getLogger(HiveReused.class.getName()).log(Level.SEVERE, null, ex);
        }
