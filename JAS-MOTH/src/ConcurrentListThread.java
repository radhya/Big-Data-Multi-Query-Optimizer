...
public  class ConcurrentListThread extends Thread {
   //public  class ConcurrentListThread implements Runnable
   public Thread t;
   public String threadName;
   public ConcurrentListThread ConcurrentThread_Arr[]= new ConcurrentListThread [10];
   //public double start_time=0;
   public double run_time=0;
   public List<String>  querylist= new ArrayList<String>();
   
    ConcurrentListThread( ){
   ...
   public  void run()  {
        ...
        try {
           ...
           // System.out.println("Running " +  this.threadName );
            try {
                //for(int i = 4; i > 0; i--) 
                //  System.out.println(" in Thread: " + threadName + ", " + i);
               
                {
                             // Let the thread sleep for a while.
                                
                            // String tn= this.threadName;
                        try {
                            try {
                                 for (int i = 0; i <querylist.size(); i++)
                                 {
                                 String connection_name=this.threadName+"-"+String.valueOf(i+1);
                                 
                                String sql=querylist.get(i);
                                 //start_time=System.currentTimeMillis()/1000;
                               // this.run_time+=connection_db(sql,connection_name);
                                 this.run_time+=Connection_DB.Connection_DB_No_Write_ResultFile(sql,connection_name);
   ...
   public double   connection_db  (String sql, String connection_name) throws SQLException, ClassNotFoundException, IOException
     ...
     System.out.println(connection_name+" In  Running:" +sql);
...
/* */   stmt.execute ("SET hive.auto.convert.join.noconditionaltask.size=25000000");/*/
  ...
  {
      List<String>  querylist= new ArrayList<String>();
        querylist.add("select * from lineitem_t2  where  l_discount> 0.01 and l_discount< 0.03");
        querylist.add("select * from lineitem_t2  where  l_discount between 0.04 and  0.08");
