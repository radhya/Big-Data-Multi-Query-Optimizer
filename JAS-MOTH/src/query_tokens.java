...
public class query_tokens {
 ...
 //  public  List <String>  where_clauses =new ArrayList <String> ();  
   ...
   public  void query_tokens()
   {
    ...
    this.where_clauses="";
    this.join_clauses="";
    //this.tempresult=  new String [1000000][10];//(LinkedList<String>[][])new LinkedList<?>[1000][10];
  
   // tempresult=    //ArrayList[1000][3];
    //LinkedList<Integer>[][] grid = (LinkedList<Integer>[][])new LinkedList<?>[5][3];
    //this.where_clauses.add(null);
   }
        ...
        // System.out.println("TCsql"+TCsql);
         ...
         if(TCsql instanceof TSelectSqlStatement)
         {
            ...
            for(int i=0;i<columns.size();i++)
            {
             ...
             //  System.out.println("columncnnnnns :"+columns.getResultColumn(i).getExpr().getExpressionType().toString());
              // System.out.println("columncnnnnns :"+columns.getResultColumn(i).getExpr().getFunctionCall().getAggregateType().name());
         ...
         //   System.out.println("columns :\n"+cols[a]);
            for(int j=0;j<select.tables.size();j++)            
            {
                   ...
                   // System.out.println("Table Name:"+table.getName());
                    ...
                    //System.out.println("Table Name in query_tokens_arr:"+query_tokens_arr[query_num].table_name);
                    //here table name 
                   // tbl=table.getName();
             ...
             if (select.getWhereClause()!=null )
            {
              ...
              //cols.add(x.getColumnNameOnly()); 
               
               y=select.getWhereClause().getCondition().getRightOperand().getObjectOperand();
               //cols.add(y.getColumnNameOnly());
               //System.out.printf("\nwhere clause: \n\t%s\n",select.getWhereClause().getCondition().toString()); 
               
               d=select.getWhereClause().getCondition().toString(); 
              /*
               System.out.println("getExpressionType:"+select.getWhereClause().getCondition().getExpressionType());
               //String LeftOperand
              System.out.println("LeftOperand :"+select.getWhereClause().getCondition().getLeftOperand());
               System.out.println("operator :"+select.getWhereClause().getCondition().getOperatorToken());
               System.out.println("RightOperand ..> value :"+select.getWhereClause().getCondition().getRightOperand());                    
               System.out.println("left left:"+select.getWhereClause().getCondition().getLeftOperand().getLeftOperand());
               System.out.println("right left:"+select.getWhereClause().getCondition().getLeftOperand().getRightOperand());
               System.out.println("left right:"+select.getWhereClause().getCondition().getRightOperand().getLeftOperand());
               System.out.println("right right:"+select.getWhereClause().getCondition().getRightOperand().getRightOperand());
               */
              ...
              // System.out.println("LeftOperand " +this.LeftOperand+ "...this.RightOperand..."+ this.RightOperand);//.get(0));
               //this.where_clauses.add(d);
...
/*4*/ //alias4=(source_join.getTable().getAliasClause() !=null)?source_join.getTable().getAliasClause().toString():"";System.out.println("amma=  "+alias4);
  ...
  /*5*/                  //   alias5=(joinItem.getTable().getAliasClause() !=null)?joinItem.getTable().getAliasClause().toString():"";System.out.println("ammmnaa=  "+alias5);   
    ...
    /*
     * if (temp[2].contains(tabs.toString()) && temp[3].contains(cols.element()) && (!sql.trim().contains("join"))&& (!sql.trim().contains("where"))&&
              (!sql.trim().contains("having"))&& (!temp[0].contains("join")) && (!temp[0].contains("where")) &&
              (!sql.trim().contains("group by"))&& (!temp[0].contains("having")) && (!temp[0].contains("group by")) &&
              (!sql.trim().contains("order by")) && (!temp[0].contains("order by")) ) //case2
